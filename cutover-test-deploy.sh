...
read -r APP_NAME TEST_APP_NAME INSTANCES <<<$(jq -r '. | "\(.app_name) \(.test_app_name) \(.instances)"' "${json_file}")
...
echo "Mapping route ${DEPLOYED_APP}${APP_SUFFIX}.${CF_APP_DOMAIN} to app ${NEW_APP}"
